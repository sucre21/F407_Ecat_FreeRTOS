...
\brief      This function will copies the inputs from the local memory to the ESC memory
            to the hardware
*////////////////////////////////////////////////////////////////////////////////////////
void PDO_InputMapping(void)           //从站发送反馈数据  ，从stm32 的内存拷贝到et1100的内存中
{
    APPL_InputMapping((UINT16*)aPdInputData);
    HW_EscWriteIsr(((MEM_ADDR *) aPdInputData), nEscAddrInputData, nPdInputSize );//用了三个元素
}
/////////////////////////////////////////////////////////////////////////////////////////
/**
...
*////////////////////////////////////////////////////////////////////////////////////////
void PDO_OutputMapping(void)         //从站读取指令 ，从et1100的内存拷贝到 stm32 的内存中
{

    HW_EscReadIsr(((MEM_ADDR *)aPdOutputData), nEscAddrOutputData, nPdOutputSize );

    APPL_OutputMapping((UINT16*) aPdOutputData);  //用了一个元素，SPI8bit传输，16bit存储，故
}
...
void PDI_Isr(void)              //数据读写函数
{
    if(bEscIntEnabled)   //中断使能，同步模式
    {
...
void MainLoop(void)
{
    ...
    if(bInitFinished == FALSE)
            ...
            )
        {
           ...
           if ( !bEscIntEnabled )//工作在自由查询模式
            {
                UINT16 ALEvent = HW_GetALEventRegister();
                ALEvent = SWAPWORD(ALEvent);  //SM2事件标志，周期性数据输出事件，输入输出是对主站来说的

                if ( ALEvent & PROCESS_OUTPUT_EVENT )//有输出事件发生
                {
                    bEcatFirstOutputsReceived = TRUE;
                    if ( bEcatOutputUpdateRunning )
                    {
                        PDO_OutputMapping(); //更新输出存储区域的数据，即从站读取指令
                    }
                }
                else if ( nPdOutputSize == 0 )//无事件发生，继续等待查询
                {
                    if ( ALEvent & PROCESS_INPUT_EVENT ) //如果发生输入事件，即从站向上发送时，复位标志位
                    {
                        bEcatFirstOutputsReceived = TRUE;
                    }
                }
            ...
            DISABLE_ESC_INT();
            ECAT_Application();//用户程序，可进行数据处理，伺服控制等

            if ( bEcatInputUpdateRunning )
            {
                PDO_InputMapping();  //发送数据反馈
            }
            ENABLE_ESC_INT(); 
        } */
	#if !ECAT_TIMER_INT
        ...
        ECAT_Main();    //处理非周期性数据，邮箱操作

        /* call lower prior application part */
       COE_Main();
       CheckIfEcatError(); //错误检测，状态机切换
