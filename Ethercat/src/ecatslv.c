...
void CheckIfEcatError(void)
{
   ...
   if (EcatWdValue != 0)
   {
      ...
      if (!(WdStatusOK & ESC_PD_WD_TRIGGER_MASK) && (nPdOutputSize > 0))
      {
         ...
         if (bEcatOutputUpdateRunning)
         {
            AL_ControlInd(STATE_SAFEOP, ALSTATUSCODE_SMWATCHDOG); //状态机切换
            return;
         }
...
void ECAT_Init(void)
{
    ...
    /*Get Maximum Number of SyncManagers and supported DPRAM size*/   //通过SPI获取et1100内SM数目和DPRAM的大小
    {
    UINT16 TmpVar = 0;
    ...
    /* Get EEPROM loaded information */   //通过SPI获取EEPROM load状态
    UpdateEEPROMLoadedState();
...
void ECAT_Main(void)
{
    ...
    if ( bMbxRunning )
    {
        ...
        HW_EscReadWord(sm1Activate,(ESC_SYNCMAN_ACTIVE_OFFSET + SIZEOF_SM_REGISTER));  //读取sm1寄存器，判断是否有邮箱重发申请
    ...
    */
    if ( bMbxRunning )
    {
        ...
        if ( ALEventReg & (MAILBOX_READ_EVENT) )
        {
            ...
            u16dummy = 0;
            HW_EscWriteWord(u16dummy,u16EscAddrSendMbx);     //写SMl配置邮箱缓存的第一个字节来响应事件：
        ...
        if ( ALEventReg & (MAILBOX_WRITE_EVENT) )
        {
            ...
            MBX_CheckAndCopyMailbox();           //邮箱发送事件处理
